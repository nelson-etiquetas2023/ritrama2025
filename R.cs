    ...
    public static class R
    {
        ...
        public class QUERY
        {
            public class PRODUCTION
            {
                ...
                internal static string SQL_QUERY_SELECT_LOAD_ROLL_ID = "SELECT a.roll_id,a.part_number,b.product_name,a.Width,lenght, ISNULL(msi,0) AS msi,disponible, fecha_pro, fecha_reg, splice,Core,Ubicacion,'Inic.' AS tipo_mov,largo_consumido, CASE WHEN (lenght - largo_consumido) <= 0 THEN 0 ELSE (lenght - largo_consumido) END AS largo_restante,CASE WHEN largo_consumido=0 THEN 'Completo' WHEN (lenght - largo_consumido)<=0 THEN 'Agotado' ELSE 'Parcialmente Consumido' END AS estado FROM MasterInic a LEFT JOIN producto b ON a.part_number = b.product_id where b.MasterRolls = 1 and a.disponible = 1 union SELECT a.rollid as roll_id,a.product_id as part_number,b.product_name,width,length,ISNULL(msi,0) AS msi,disponible=1,c.fecha_pro,c.fecha_recepcion as fecha_reg,splice,Core,ubicacion,'Importacion' AS tipo_mov,a.largo_consumido,a.largo_restante,a.estado FROM ItemsMateria a LEFT JOIN producto b ON a.product_id= b.product_id LEFT JOIN OrdenMateria c ON a.numero=c.numero WHERE b.MasterRolls=1";
