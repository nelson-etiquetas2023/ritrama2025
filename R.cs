    ...
    public static class R
    {
        ...
        public class QUERY
        {
            public class PRODUCTION
            {
                ...
                internal static string SQL_QUERY_SELECT_LOAD_ROLL_ID = "with detalle_consumo as (SELECT (x.consumo + case when x.desperdicio = 1 then (x.leng-x.consumo) else 0 end) as ConsumoTotal,x.rollid,x.desperdicio,x.anulada,x.numero,x.TwoMaster from orden_corte oc CROSS APPLY (VALUES(oc.util1_real_lenght,oc.rollid_1,desperdicio,lenght_1,anulada,numero,TwoMasters),(oc.util2_real_lenght,oc.rollid_2,desperdicio2,lenght_2,anulada,numero,TwoMasters)) AS x(consumo,rollid,desperdicio,leng,anulada,numero,TwoMaster) where x.consumo > 0 and x.anulada=0) select t.Roll_Id,t.Part_Number,t.Product_Name,t.Width,t.Lenght,t.msi,t.fecha_pro,t.fecha_reg,t.Splice,t.Core,t.Ubicacion,t.tipo_mov,t.largo_consumido,(t.Lenght-t.largo_consumido) as largo_restante,CASE WHEN t.largo_consumido=0 THEN 'Completo' WHEN (t.Lenght - t.largo_consumido)<=0 THEN 'Agotado' ELSE 'Parcialmente Consumido' END AS estado from (select a.Roll_Id,a.Part_Number,b.Product_Name,a.Width,a.Lenght,ISNULL(a.msi,0) AS msi,a.fecha_pro,a.fecha_reg,a.splice,a.core,a.Ubicacion,'Inic.' AS tipo_mov,ISNULL((select sum(x.ConsumoTotal) from detalle_consumo x where a.Roll_Id = x.rollid),0) as largo_consumido from MasterInic a LEFT JOIN producto b ON a.Part_Number = b.product_id where b.MasterRolls = 1) as t UNION ALL SELECT t.rollid,t.product_id,t.Product_Name,t.Width,t.length,t.msi,t.fecha_produccion,t.fecha_llegada,t.Splice,t.Core,t.Ubicacion,t.tipo_mov,t.largo_consumido,(t.length - t.largo_consumido) AS largo_restante,CASE WHEN t.largo_consumido = 0 THEN 'Completo' WHEN (t.length - t.largo_consumido) <= 0 THEN 'Agotado' ELSE 'Parcialmente Consumido' END AS estado FROM (SELECT a.rollid,a.product_id,b.Product_Name,a.Width,a.length,ISNULL(a.msi,0) AS msi,a.fecha_produccion,a.fecha_llegada,a.splice,a.core,a.Ubicacion,'Por Compras' AS tipo_mov,ISNULL((SELECT SUM(x.ConsumoTotal) FROM detalle_consumo x WHERE a.rollid = x.rollid),0) AS largo_consumido FROM ItemsMateria a LEFT JOIN producto b ON a.product_id = b.product_id WHERE b.MasterRolls = 1) AS t order by t.Roll_Id";
