...
namespace Ritrama2025
{
    public static class R
    {
        ...
        {
            internal static string NAME_RELATION_OC_MASTER_DETAILS = "RELATION_MASTER_DETAILS_ROLLOS_CORTADOS";
        }
        public class QUERY
        {
            public class PRODUCTION
            {
                ...
                internal static string SQL_QUERY_SELECT_LOAD_OC_ROLLO_CORTADO = "SELECT numero,product_id,product_name,roll_number,unique_code,splice,width,large,msi,roll_id,code_person,status,disponible,width_c,lenght_c,ubic,ratio,fecha,rollid_oculto FROM rolls_details ORDER BY numero DESC";
                internal static string SQL_QUERY_SELECT_LOAD_ROLL_ID = "SELECT a.roll_id,a.part_number,b.product_name,a.master,a.graphics,a.resma,a.Width,lenght=a.lenght - a.lenght_c, disponible, fecha_pro, fecha_recep, splice, Ubicacion,'Inic.' AS tipo_mov,largo_consumido,(lenght - largo_consumido) as largo_restante,CASE WHEN largo_consumido=0 THEN 'Completo' WHEN (lenght - largo_consumido)=0 THEN 'Agotado' ELSE 'Parcialmente Utilizado' END AS estado FROM MasterInic a LEFT JOIN producto b ON a.part_number = b.product_id where b.MasterRolls = 1 and a.disponible = 1";
        ...
        public static class SQL_STRING_QUERY
        {
            ...
            internal readonly static string SELECT_QUERY_MP_DETAILS = "select numero,product_id,type,cant_pedido,cant_real,width,length,msi,rollid,splice,ubicacion,core from ItemsMateria";
